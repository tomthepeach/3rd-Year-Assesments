...
bool forest_fire(int N, double p, int &steps, char wind_direction= '0')
{
	/* 
...
int main()
{
	...
	bool wind = getenv("WIND");
	if (wind)
	{
		// set wind direction to 'N' , 'S' , 'E' or 'W' with ~ equal probability

		int wind_direction_int = rand() % 4;
		if (wind_direction_int == 0) wind_direction = 'N';
		if (wind_direction_int == 1) wind_direction = 'S';
		if (wind_direction_int == 2) wind_direction = 'E';
		if (wind_direction_int == 3) wind_direction = 'W';
