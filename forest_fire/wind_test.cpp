...
using std::cout, std::endl, std::getenv, std::ofstream, std::string, std::stoi,std::vector, std::srand, std::to_string;
...
bool forest_fire(int N, double p, int &steps, char wind_direction= 'S')
{
	/* 
	...
	*/ 
	...
	while (burning)
	{
        ...
        ofstream output;
        string filename = "testdata/windtest_" + to_string(t) + "_test.csv";
...
int main()
{
    ...
    int nsteps;
    bool bottom_reached = forest_fire(100, 0.6, nsteps);
